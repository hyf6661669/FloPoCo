...
\resizebox{!}{100pt}{
  \begin{tikzpicture}
	...
	\only<1,3->{
	\node[draw, thick, dotted, rectangle, minimum width = 6cm, minimum height=3.5cm, align=left, green!100, fill=green!20] (sop1) at (2cm,-2.5cm) {};
	}
	\only<3->{
	\node[draw, thick, dotted, rectangle, minimum width = 4cm, minimum height=3.5cm, align=left, green!100, fill=green!20] (sop2) at (7.5cm,-2.5cm) {};
	}
...
%      \draw[hwbus, ->] ($(8*\i ex, 0ex)$) --  ++(0, -5ex);
    ...
    \only<1,4>{\draw[hwbus, <-] (40,-15) -- ++(-10,0) node [near end,xshift=-5mm] {\only<4>{/}} node [near end,below, text width=4cm, xshift=0.5cm] {\only<3>{\footnotesize$(msb_{out},$ $ lsb_{out}+g)$}} node[near start,above] {\only<3>{$y^*(k)$}} -- ++(-1,0) ;
	}
	...
	\only<1,3->{
     \node[green!50!black]  at (-0.5cm,-4cm) {{\small SOPC}};   
	}

	\only<3->{
     \node[green!50!black]  at (6.0cm,-4cm) {{\small SOPC}};   
	}
