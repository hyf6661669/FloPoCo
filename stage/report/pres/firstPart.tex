\subsection{Fundamentals, Purpose, Definitions}
...
\resizebox{!}{100pt}{
  \begin{tikzpicture}
	...
	\only<2->{
     \node[green]  at (-2ex,-17.5ex) {{\small SOPC}};   
	}
	\only<1,3->{
	\node[draw, thick, dotted, rectangle, minimum width = 6cm, minimum height=3.5cm, align=left, green!100, fill=green!30] (sop1) at (2cm,-2.5cm) {};
	}
	\only<3->{
	\node[draw, thick, dotted, rectangle, minimum width = 4cm, minimum height=3.5cm, align=left, green!100, fill=green!30] (sop2) at (7.5cm,-2.5cm) {};
	}
...
%      \draw[hwbus, ->] ($(8*\i ex, 0ex)$) --  ++(0, -5ex);
    ...
    \only<1>{\draw[hwbus, <-] (40,-15) -- ++(-10,0) node [near end] {\only<5>{/}} node [near end,below, text width=4cm, xshift=0.5cm] {\only<5>{\footnotesize$(msb_{out},$ $ lsb_{out}+g)$}} node[near start,above] {$y^*(k)$} -- ++(-1,0) ;
	}
    \only<2->{\draw[hwbus, <-] (70,-15) -- ++(-15,0) node [near end] {\only<4>{/}} node [near end,below, text width=4cm, xshift=1.5cm] {\only<4>{\footnotesize$(msb_{out},$ $ lsb_{out})$}} node[near start,above] {$y^*(k)$} -- ++(-1,0) ;
	}
    ...
    \draw[hwbus,<-] ($(60 , -5)$) --  ++(0,-5);
     \node[green!100]  at (-0.5cm,-4cm) {};% node for keeping scale
	}
	\only<1,3->{
     \node[green!100]  at (-0.5cm,-4cm) {{\small SOPC}};   
	}

	\only<3->{
     \node[green!100]  at (6.0cm,-4cm) {{\small SOPC}};   
	}
...
Abstract architecture for the direct form realization of an \only<1>{FIR}\only<2->{IIR} filter
\end{figure}
	}
\end{frame}
	...
	\frametitle{Workflow}
		...
		\resizebox{300pt}{!}{
		  ...
		  \begin{tikzpicture}[]
		   ...
		   \node[draw, fill = gray!20, shape = rectangle, minimum width = 2cm, minimum height= 2cm] at (-4cm,4cm) {FixSIF} ;
		   \node[draw, fill = gray!20, shape = rectangle, minimum width = 1cm, minimum height= 1.2cm] at (-1cm,4cm) {VHDL} ;
			...
			\draw (-5.5cm, 0.7cm) -- (-3.0cm,0.7cm) [->, thick] node[below,xshift=-1.2cm]{$\begin{pmatrix}\boldsymbol{-J} & \boldsymbol{M} & \boldsymbol{M}_t \\ \boldsymbol{K} & \boldsymbol{P} & \boldsymbol{M}_x \\ \boldsymbol{L} & \boldsymbol{R} & \boldsymbol{M}_y \end{pmatrix}$};
			\draw (-8cm,0.0cm) rectangle ++(2.5cm,1.4cm)[thick] node [midway, text width = 4cm, align=center]{Realization Choice \\ by \\ Hilaire, Volkova  \\ team PEQUAN, LIP6 \\(Front-End)}; 
			\draw (-3.0cm,0.0cm) rectangle ++(3.0cm,1.4cm)[thick] node [midway, text width = 3cm, align=center]{Flopoco core generation \\ (this work) \\(Back-End)}; 

			%\draw (1cm,0cm) rectangle ++(2.5cm,1.4cm)[thick] node [midway]{VHDL}; 
