	...
	Dealing with MIMO filters, we have $\boldsymbol{h}(k) \in \mathbb{R}^{n_y \times n_u}$ as the impulse response of $\mathcal{H}$. $\boldsymbol{h}_{i,j}(k)$ is the response on the
	...
	Most of the time in this report, we may refer to realizations as forms.
...
\end{figure}
	...
	Indeed, when outputs loop back on inputs, as soon as we are in finite precision, the error is amplified by a certain amount, depending on the coefficients, at each pass through the filter.
	...
	Fortunately filters we are considering have an error amplification bounded, which allows us to use filters WCPGs.

	Here we have to compute each size at each step of the computation. Of course, the WCPG is not useful for the first part of a FIR, as it includes no loop.
	...
	As $\boldsymbol{\varepsilon}$-errors come only from the SOPCs cores, we have another error quantum that will symbolize the total error.
		...
		In the future we will get rid of $\boldsymbol{\delta y}_m$.
