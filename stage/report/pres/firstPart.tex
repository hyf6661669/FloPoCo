...
\begin{frame}
	\frametitle{FIR, IIR}
	\only<1,2> {The most simple LTI Filter: the FIR (Finite Impulse Response) filter}
	\only<3-> {Full LTI complexity: the IIR (Infinite Impulse Response) filter:}
	\begin{equation} \label{iirdef}
		\only<1,2>{y(k)=\sum_{i=0}^n b_i u(k-i)}
		\only<3->{y(k)=\sum_{i=0}^n b_i u(k-i) - \sum_{i=0}^n a_i y(k-i)}
	\end{equation}


\only<2>{Abstract architecture for the direct form realization of \only<2>{a FIR}\only<3->{an IIR} filter:}
	\only<2->{
\begin{figure}[]
\resizebox{!}{100pt}{
  \begin{tikzpicture}
...
%      \draw[hwbus, ->] ($(8*\i ex, 0ex)$) --  ++(0, -5ex);
    ...
    \foreach \i in {1,...,3} {
      ...
      % The registers
      \draw[hwbus] ($(-8*\i  + 60  +8, 0)$) --  ++(-8, 0) node [above,text=blue] {\footnotesize $y\only<5>{^*}(k-\i)$};
      \draw[hwbus,->] ($(-8*\i  + 60  +8, 0)$) --  ++(-3, 0); % just for the arrow
      \draw[line width=3pt] ($(-8*\i  +60 + 4, -3)$) --  +(0, 6);
    }
	}
    \only<2>{\draw[hwbus, <-] (40,-15) -- ++(-10,0) node [near end,xshift=-5mm] {\only<5>{/}} node [near end,below, text width=4cm, xshift=0.5cm] {\only<3>{\footnotesize$(msb_{out},$ $ lsb_{out}+g)$}} node[near start,above] {\only<2>{$y(k)$}} -- ++(-1,0) ;
	}
    \only<3->{\draw[hwbus, <-] (70,-15) -- ++(-15,0) node [near end] {\only<5>{/}} node [near end,below, text width=4cm, xshift=1.5cm] {\only<5>{\footnotesize$(msb_{out},$ $ lsb_{out})$}} node[near start,above] {$y\only<5>{^*}(k)$} -- ++(-1,0) ;
	}
