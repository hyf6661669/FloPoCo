...
ConfigurationGenerator::ConfigurationGenerator(Target* target, int coeffWordSize_) : Operator(target), coeffWordSize(coeffWordSize_) {
    ...
    {
        //use coeff2lut_prim.vhd

        vhdl << "dut1: entity work.coeff2lut_prim"<< endl;
        vhdl << "generic map(Bc => " <<  std::to_string(coeffWordSize) <<")"<< endl;
        vhdl << "port map( "<< endl;
        vhdl << "clk_i => clk,"<< endl;
        vhdl << "rst_i => rst, "<< endl;
        vhdl << "ce_i => ce_i, "<< endl;
        vhdl << "coeff_i => coeff_i, "<< endl;
        vhdl << "ctrl_int => ctrl_int, "<< endl;
        vhdl << "cdo_lsb_o => cdo_lsb_o, "<< endl;
        vhdl << "cdo_msb_o => cdo_msb_o );"<< endl;
    }
    else
    {
        //use coeff2lut_no_prim.vhd
        vhdl << "dut1: entity work.coeff2lut_prim "<< endl;
        vhdl << "generic map(Bc => " << std::to_string(coeffWordSize) <<")"<< endl;
        vhdl << "port map(  "<< endl;
        vhdl << "clk => clk,"<< endl;
