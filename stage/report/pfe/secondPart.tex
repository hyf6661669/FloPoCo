...
\begin{figure}
 ...
 \end{tikzpicture}
\end{center}
\caption{KCM-based SOPC architecture for $n_c=4$, each input being split into 3 chunks  \label{fig:Overall architecture}}
\end{figure}

	\subsubsection[Precision concerns]{Precision concerns\footnote{For more details about precision calculations, please see appendix \ref{preccal}}}
...
%	The idea now is to see what and be done in arbitrary precision, trying to save a maximum of logic while keeping a right result on the precision required by the user.
	The following part comes back over calculations from Lopez's PhD \cite{lopez}, with adaptations in the architecture context.
...
%			This new algorithm corresponds here to the algorithm of the SIF of a filter $\mathcal{H}_{\boldsymbol{\varepsilon}}$,
			...
			\begin{itemize}
				\item $\mathcal{H}$ the absolute filter in infinite precision \vspace{-5pt}
				\item $\mathcal{H}_{\boldsymbol{\varepsilon}}$ the error filter \vspace{-5pt}
			\end{itemize}
...
%		That is, 
		...
		And $v\in\{t,x,y\}$.
		 ...
		 So, all $lsb_{v_i}$ from all $msb_{v_i}$ can  be deduced from \ref{constraint}.
...
\subsection{Architecture generation algorithm}
	...
	\begin{figure}[!h]
	\begin{center}
	\scalebox{6.6}{\input{pictures/example.tex}}
	\end{center}
	\caption{ Architecture implementing a SIF (example from a rho-DFII filter), with $n_t=5$, $n_x=5$ and $n_y=1$ \label{fig:SIFimpl}}
	\end{figure}
	...
	On the architecture scheme: \vspace{-7pt}
	\begin{itemize}
