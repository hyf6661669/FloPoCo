...
namespace flopoco {
    Xilinx_GenericAddSub_slice::Xilinx_GenericAddSub_slice( Target *target, int wIn, bool initial, bool fixed, bool dss ) : Operator( target ) {
    ...
    void Xilinx_GenericAddSub_slice::build_normal( Target *target, int wIn, bool initial ) {
        ...
        addInput( "x_in", wIn );
        addInput( "y_in", wIn );
        addInput( "neg_x_in", wIn );
        addInput( "neg_y_in", wIn );
        addInput( "carry_in", wIn );
        addOutput( "carry_out", wIn );
        ...
        if ( wIn < 4 ) {
            ...
            vhdl << tab << "cc_s" << range( wIn - 1, 0 ) << " <= lut_o5;" << std::endl;
        } else {
            vhdl << tab << "cc_di" << " <= lut_o5;" << std::endl;
            vhdl << tab << "cc_s" << " <= lut_o5;" << std::endl;
        }
