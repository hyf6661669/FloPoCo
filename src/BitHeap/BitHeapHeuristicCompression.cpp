...
namespace flopoco
{
    ...
    int BitHeapHeuristicCompression::generateProblem(){
        ...
        if(mode.compare("heuristic_full") == 0 || mode.compare("heuristic_no_bits_passed") == 0){
            //here the idea is to use as many (double) (6,0,6;5) compressors as possible

            bool usePrevStages;
            if(mode.compare("heuristic_full") == 0){
                usePrevStages = true;
            }
            else{
                //no PrevStages if "heuristic_no_bits_passed"
                usePrevStages = false;
            }

            for(int s = 0; s < noOfStages_ - 1; s++){

                bool found = true;

                while(found){

                    //idea is to first look aside in the bitheap. therefore found is used
                    found = false;

                    //check if we can use the compressors anywhere as a block
                    for(unsigned c = 0; c < newBits[s].size(); ){
                        int noOfColumns = noOfColumnsFor606(s, c, usePrevStages, false);

                        if(noOfColumns >= 4){
                            found = true;

                            //use compressor
                            int noOfReplacingCompBlocks = noOfColumns / 4;

                            //now decide where to start replacing
                            int columnsNotUsed = noOfColumns % 4;

                            if(columnsNotUsed == 0){
                                use606(s, c, noOfReplacingCompBlocks, usePrevStages, false);
                            }
                            else if(columnsNotUsed == 1 || columnsNotUsed == 2){
                                //leave lowest column (and at columnsNotUsed == 2 the highest column) untouched
                                use606(s, c , noOfReplacingCompBlocks, usePrevStages, false);
                            }
                            else{
                                //columnsNotUsed == 3
                                //leave the lowest two and the highest column untouched
                                use606(s, c, noOfReplacingCompBlocks, usePrevStages, false);
                            }
                ...
                //now use single (6,0,6;5) compressors

                for(unsigned c = 0; c < newBits[s].size(); c++){
                    int useCompressor = 1;
                    while(useCompressor == 1){
                        useCompressor = noOfColumnsFor606(s, c, usePrevStages, true);
                        if(useCompressor){
                            use606(s, c, 1, usePrevStages, true);
                        }
                    }
                }

                //debug output:: newBits
                cout << "newbits after stage " << s << ":" << endl;
                for(unsigned i = 0; i < newBits.size(); i++){
                    for(unsigned j = 0; j < newBits[i].size(); j++){
                        cout << newBits[i][j] << " ";
                    }
        ...
        else if(mode.compare("heuristic_parandeh-afshar_modified") == 0){
...
//                            zeros = s - 1;
    ...
    //checks how many consecutive columns starting with c in stage s have the size >= 6 and returns the amount
    //if useSingleCompressor = true, it only checks whether we can use a single compressor here and returns 1 if we can
    int BitHeapHeuristicCompression::noOfColumnsFor606(int s, int c, bool usePreviousStages, bool useSingleCompressor){
        int result = 0;
        unsigned column = c;
        bool found = true;
        if(!useSingleCompressor){
            while(found == true && column < newBits[s].size()){


                found = checkColumnFor6Bits(s, column, usePreviousStages);
            ...
            //only single compressor therefore check at column and column + 2
            if(checkColumnFor6Bits(s, column, usePreviousStages) &&
                    checkColumnFor6Bits(s, column + 2, usePreviousStages)){
                return 1;
            }
            else{
                return 0;
    ...
    bool BitHeapHeuristicCompression::checkColumnFor6Bits(int s, int column, bool usePreviousStages){


        if(!usePreviousStages){
            if(newBits[s][column] >= 6){
                return true;
            }
            else{
                return false;
            ...
            //use bits from previous stages for (6,0,6;5) compressor as well
            int sum = 0;


            //lowestStage is the stage, where for this column a stage still exists
            //thats necessary because we increase the number of columns in every stage by the maxOutput of the compressors
            int lowestStage;
            for(int i = s; i >= 0; i--){
                if(newBits[i].size() > (unsigned) column){
                    lowestStage = i;
                }
            }

            for(int i = s; i >= lowestStage; i--){
                sum += newBits[i][column];
            }
            if(sum >= 6){
                return true;
            }
    ...
    int BitHeapHeuristicCompression::use606(int s, int start, int times, bool usePreviousStages, bool useSingleCompressor){
        int result = 0;
        int c = start;
        int t = times;

        if(!useSingleCompressor){
            //only blocks of two (6,0,6;5) combined

            while(t > 0){

                if(!usePreviousStages){
                    //remove bits from current stage -(6666)
                    newBits[s][c]       -= 6;
                    newBits[s][c + 1]   -= 6;
                    newBits[s][c + 2]   -= 6;
                    newBits[s][c + 3]   -= 6;
                    ...
                    //remove bits from current or previous stages
                    removeBitsIncludePreviousStages(s, c, 6);
                    removeBitsIncludePreviousStages(s, c + 1, 6);
                    removeBitsIncludePreviousStages(s, c + 2, 6);
                    removeBitsIncludePreviousStages(s, c + 3, 6);
                }


                //add bits to next stage +(12221)
                newBits[s + 1][c]       += 1;
                newBits[s + 1][c + 1]   += 2;
                newBits[s + 1][c + 2]   += 2;
                newBits[s + 1][c + 3]   += 2;
                ...
                //add compressors to solution. it is compressor no. 8
                solution[s].push_back(pair<int,int>(8,c));
                solution[s].push_back(pair<int,int>(8, c+1));
            ...
            //check whether replacements where successfull
            if(result == times){
                return 0;
            }
            ...
            //use one single (6,0,6;5) in column c
    ...
    int BitHeapHeuristicCompression::removeBitsIncludePreviousStages(int s, int c, int bitsToRemove){
        //remove bits from this stage and from previous stages
        //fill solution with flipflops if it is necessary
        int remainingBits = bitsToRemove;

        for(unsigned i = s; i >= 0; i--){
            int removedBits = 0;

            if(newBits[i][c] >= remainingBits){
                newBits[i][c] -= remainingBits;
                removedBits = remainingBits;
                remainingBits = 0;
            }
            else{
                if(newBits[i][c] <= remainingBits){
                    remainingBits -= newBits[i][c];
                    removedBits = newBits[i][c];
                    newBits[i][c] = 0;
                }
                else{
                    newBits[i][c] -= remainingBits;
            ...
            if((unsigned) removedBits > 0 && i != (unsigned) s){
                //add removedBits-times flipflops in every stage until s
                //starting from the stage where the bit(s) is found
                for(unsigned j = i; j < (unsigned) s; j++){
                    for(unsigned k = 0; k < (unsigned) removedBits; k++){
                        solution[j].push_back(pair<int,int>(13,c));
                    }
                }
            }
