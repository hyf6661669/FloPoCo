...
void lut_op::print(string pre){
    switch( type ){
        case LUT_OR: cout << "LUT_OR";break;
        case LUT_AND: cout << "LUT_AND";break;
        case LUT_NOT: cout << "LUT_NOT";break;
        case LUT_XOR: cout << "LUT_XOR";break;
        case LUT_IN: cout << "LUT_IN(" << id << ")";break;
        default: cout << "LUT_NOOP";break;
    }
    cout << endl;
...
bool lut_op::eval( bool *in ) {
    switch( type ){
    case LUT_OR: return eval_or(in);
    case LUT_AND: return eval_and(in);
    case LUT_NOT: return eval_not(in);
    case LUT_XOR: return eval_xor(in);
    case LUT_IN: return eval_in(in);
    default:
        if(el.size()>0)
            return el[0].eval(in);
        else
