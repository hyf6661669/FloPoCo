    ...
    class Xilinx_GenericAddSub : public Operator {
      ...
      public:
        ...
        static OperatorPtr parseArguments( Target *target, vector<string> &args ){
            ...
            if( dss ){
                return new Xilinx_GenericAddSub(target,wIn,dss);
            }else if( mode > 0 ){
                return new Xilinx_GenericAddSub(target,wIn,mode);
            }else{
                return new Xilinx_GenericAddSub(target,wIn,false);
            }
        }
        static void registerFactory(){
            UserInterface::add( "xilinx_addsub", // name
                                "A adder/subtractor build of xilinx primitives.", // description, string
