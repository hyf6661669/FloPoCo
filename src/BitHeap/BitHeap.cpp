...
*/
    ...
    void BitHeap::generateCompressorVHDL(bool redundantOutput)
	{
...
/* Uni KS stop */
			...
			else
			{
				plotter->heapSnapshot(true,  plottingCycle, plottingCP);
//				if(compressionType == 0) //??
                generateFinalAddVHDL(true,redundantOutput);
			}
...
*/
	...
	//the final addition
    void BitHeap::generateFinalAddVHDL(bool isXilinx, bool redundantOutput)
	{
        ...
        if(isXilinx)
		{
            ...
            if(redundantOutput)
            {
                op->vhdl << tab << op->declare(join("CompressionResult", guid,1), maxIndex-minIndex) << " <= " << inAdder1.str() << endl;

                int tempR_bh_width=0;
                for(int i=chunkDoneIndex-1; i>=0; i--)
                    tempR_bh_width += op->getSignalByName(join("tempR_bh", guid, "_", i))->width();

                op->vhdl << tab << op->declare(join("CompressionResult", guid,0), maxIndex-minIndex+tempR_bh_width) << " <= " << inAdder0.str().substr(0,inAdder0.str().length()-1);
            }
            else
            {
                //result
