...
\begin{figure}
  \begin{tikzpicture}[scale=1.0, every node/.style={scale=1.0}]
    \footnotesize 
     \draw[green, fill=green!20] (-0.5cm,-2.2cm) rectangle +(10cm,4.4cm);   
	 \node[green!50!black] at(0cm, -2cm) {SOPC};
    ...
    \draw[hwbus,->] (sum.south) --  ++(0,-12ex) node[near start, yshift=-1mm] {$/$} node[near start,left, yshift=-1mm] {\normalsize ($msb_{out}-lsb{out}$)} node[near end,right] {$y$};
 \end{tikzpicture}
\end{figure}
\centering
SOPC architecture
