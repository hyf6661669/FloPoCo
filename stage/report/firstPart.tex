	...
	into a signal $\boldsymbol{y} = \mathcal{H}(\boldsymbol{u})$, of size $dim(\boldsymbol{y}) = n_y$ . The case where $n_u = n_y = 1$ is referred as Single Input Single Output
...
\end{figure*}
	...
	The SIF can of course be bound with the ABCD (classic state-space) form, which gives:
	...
	\subsubsection{Optimal SIF realization}
		...
		The optimality of the SIF description is not concerned by this work.
...
\end{figure}
	...
	When there is many potential realizations for a single LTI filter, the choice of one realization
	...
	Following the same model, an error quantum for each SOPC is involved in the computation.
	...
	As $\boldsymbol{\varepsilon}$-errors come only from the SOPCs cores, another error quantum will symbolize the total error.
			Let's denote:
			\begin{eqnarray}
		...
		In the future $\boldsymbol{\delta y}_m$ won't be taken into account.
		...
		So, for computations in arbitrary precision, it is a sufficient condition (\ref{constraint}) to implement the filters.
