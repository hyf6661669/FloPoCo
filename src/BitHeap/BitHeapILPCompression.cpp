...
//		int LARGE_NUMBER = bh_->getMaxHeight()+1; //!!!
        ...
        if(useVariableCompressors){
            //constraints C5 and C6 for variable compressors. this is now suited only for RCA because there is no middle-compressor needed
            ...
            for(unsigned s = 0; s < compCountVars.size() - 1; s++){
                ...
                //make sure that the high compressor isnt used in the lowest column.
...
*/
    ...
    void BitHeapILPCompression::buildVariableCompressors(){
        if(useVariableCompressors){
            variableBasicCompressor c3_1;
            c3_1.areaCost = 1.0;
            c3_1.height = vector<int> (1);
            c3_1.outputs = vector<int> (1);
            c3_1.height[0] = 3;
            c3_1.outputs[0] = 1;
            variableBCompressors.push_back(c3_1);

            variableBasicCompressor c2_1;
            c2_1.areaCost = 1.0;
            c2_1.height = vector<int> (1);
            c2_1.outputs = vector<int> (1);
            c2_1.height[0] = 2;
            c2_1.outputs[0] = 1;
            variableBCompressors.push_back(c2_1);

            variableBasicCompressor c0_1;
            c0_1.areaCost = 0.0;
            c0_1.height = vector<int> (1);
            c0_1.outputs = vector<int> (1);
            c0_1.height[0] = 0;
            c0_1.outputs[0] = 1;
            variableBCompressors.push_back(c0_1);
        }
