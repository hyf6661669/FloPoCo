...
namespace flopoco{
	...
	public:
		...
		FixSinPoly(Target* target, int msbIn_, int lsbIn_, bool truncated = false, int msbOut_ = 0, int lsbOut_ = 0, bool signedInput_ = false, std::map<std::string, double> inputDelays = emptyDelayMap);
		...
		FixSinPoly(Operator* parentOp, Target* target, Signal* multiplicandX, int msbIn_, int lsbIn_, int truncated, int msbOut_, int lsbOut_,
							 BitHeap* bitheap,
							 bool signedInput_ = false, std::map<std::string, double> inputDelays = emptyDelayMap);
		~FixSinPoly();
