...
\begin{figure}
  \begin{tikzpicture}[scale=1.0, every node/.style={scale=1.0}]
    \footnotesize 
     \draw[green, fill=green!20] (-0.5cm,-2.2cm) rectangle +(10cm,5cm);   
	 \node[green] at(0cm, -2cm) {SOPC};
    ...
    \draw[hwbus,->] (sum.south) --  ++(0,-6ex) node[near start, yshift=-1mm] {$/$} node[near start,left, yshift=-1mm] {\normalsize$msb_{out}-lsb{out}$} node[near end,right] {$y$};
 \end{tikzpicture}
\end{figure}
\centering
...
\begin{frame}
	...
	\begin{tikzpicture}[domain=0:7.5, samples=1000]
		...
		\draw[-,dotted] (27,-1.0) -- (29,-1.0) node[right, yshift=5mm, xshift=-4mm] {\tiny wcpg};
