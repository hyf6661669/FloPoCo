	...
	The hardware implementation of anIIR can be seen on figure \ref{iirpic}

\begin{figure*}[h] \label{iirpic}
  \centering
  \begin{tikzpicture}
	...
	\subsubsection{Workflow for filter implementation}
		...
		\begin{figure}[h] 
		  ...
		  \begin{tikzpicture}[x=1cm,y=1cm]
			...
			\draw (-8,0.0) rectangle ++(4.5,2)[thick] node [midway, text width = 4cm, align=center]{Realization Choice \\ by \\ Hilaire, Volkova  \\ team PEQUAN, LIP6}; 
...
%		\end{eqnarray}
	...
	As $\boldsymbol{\varepsilon}$-errors come only from the SOPCs cores, another error term will symbolize the total error.
		...
		In the following $\boldsymbol{\delta y}_m$ won't be taken into account.
