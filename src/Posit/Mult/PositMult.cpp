...
namespace flopoco {
	...
	std::string toBinary2(int n, int len){
	    std::string r;
	...
	// ========================================================================|
		...
		outmapX << "Sign=>sign_X,Reg=>reg_X,Exp=>exp_X,Frac=>frac_X,z=>z_X,inf=>inf_X,Abs_in=>open";
		...
		outmapY << "Sign=>sign_Y,Reg=>reg_Y,Exp=>exp_Y,Frac=>frac_Y,z=>z_Y,inf=>inf_Y,Abs_in=>open";
	...
	// ========================================================================|
		...
		int mult_size = 2 * FracSize;
		vhdl << declare("frac_mult", mult_size) << " <= frac_X * frac_Y;" << endl;
		...
		vhdl << declare(target->adder3Delay(RegSize+wES+1), "sf_mult", RegSize+wES+1) << " <= (sf_X(sf_X'high) & sf_X) + (sf_Y(sf_Y'high) & sf_Y) + ovf_m;" << endl;
	...
	// ========================================================================|
		...
		if (wES > 0){
			vhdl << declare(.0, "FinalExp", wES) << " <= sf_mult" << range(wES - 1, 0) << ";" << endl;
		}
		...
		std::string maxReg = toBinary2(width-2, RegSize);
		...
		*/
		vhdl << declare(target->adderDelay(RegSize), "reg_ovf", 1, false) << " <= '1' when RegimeAns > " << maxReg << " else '0';" << endl;
		...
		/*
		if (wES > 0){ 
			vhdl << declare(target->logicDelay(2), "ovf_exp", 1, false) << " <= reg_ovf OR (NOT nzero);" << endl;
	...
	// ========================================================================|
		...
		vhdl << "with sf_sign select " << declare(target->logicDelay(), "input_shifter", 2+wES+mult_size-1) << "<=" << endl <<
			tab << "'0' & nzero ";
			if (wES > 0){
				vhdl << tab << "& FinalExp ";
			}
				vhdl << tab << "& normFrac" << range(mult_size-2, 0) << " when '1'," << endl <<
			tab << "nzero & '0' ";
			if (wES > 0){
				vhdl << tab << "& FinalExp ";
			}
				vhdl << tab << "& normFrac" << range(mult_size-2, 0) << " when '0'," << endl <<
			tab << "\"" << string(2+wES+mult_size-1, '-') << "\" when others;" << endl;


		vhdl << "with sf_sign select " << declare(target->adderDelay(RegSize-1), "shift_offset", RegSize-1) << " <=" << endl <<
	...
	// ========================================================================|
		...
		vhdl << declare(target->adderDelay(shift_size-(width-1)-2), "S_bit", 1, false) << " <= S_bit_tmp when shifted_frac" << range(shift_size-(width-1)-3, 0) << " = \"" << string(shift_size-(width-1)-2, '0') << "\" else '1';" << endl;

		
		vhdl << "with reg_ovf select " << declare(target->logicDelay(3), "round", 1, false) << "<=" << endl <<
			tab << "'0' when '1'," << endl <<
		...
		vhdl << "with sign select " << declare(2* target->adderDelay(width-1) + target->logicDelay(), "result", width) << "<=" << endl <<
	...
	void PositMult::emulate(TestCase * tc) {
		// FIX THIS: mpfr_mul produces underflow
