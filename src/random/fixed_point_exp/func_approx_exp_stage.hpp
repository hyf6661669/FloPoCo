...
public:
		...
		, m_outputResultType(resultFracWidth)
	{
		...
		function <<"(exp("<<expMu<<" + x * "<<scaleInput<<" * "<<expSigma<<")-1)*"<<scaleOutput;
		std::cerr<<" function = "<<function.str()<<"\n";
		flopoco::HOTBM *fe;
		
		fe = new flopoco::HOTBM(target, function.str(), "FAE", m_inputResidualType.Width(), m_outputResultType.FracWidthNonZero(), 1, false);
		oplist.push_back(fe);
		...
		vhdl<<tab<<"oResult <= \"";
		for(int i=1;i<OutputResultType().Width()-OutputResultType().FracWidthNonZero();i++)
			vhdl<<"0";
		vhdl<<"\"  & result;\n";
	}
