...
namespace flopoco {
    ...
    : Operator(parentOp,  target), wIn(wIn_), modulo(modulo_), method(method_) {
        ...
        if (method == "modOp" || method == "ex" || method == "redOnly") {
            ...
            vhdl << tab << declare(
                    "RedRes", modSize, false) << tab << " <= T_" << shiftSize << range(modSize, 1) << " when UNSIGNED(T_" << shiftSize << range(wIn-length_red,1) <<  ") < M else TResMM" << ";" << endl;

            // montgomery multiplication to convert back from montgomery form

            //Constant to shift back from montgomery form
            mpz_class shiftConst = (((mpz_class)1 << 2*shiftSize) % modulo);
            vhdl << tab << declare(
            ...
            vhdl << tab << "S <= R_" << shiftSize << range(modSize-1,0) << " when UNSIGNED(R_" << shiftSize << ") < M";
            vhdl << tab << "else STD_LOGIC_VECTOR(UNSIGNED(R_" << shiftSize << range(modSize-1,0) << ") - M);" << endl;
        ...
        UserInterface::add("ModuloMontgomery", // name
                ...
                // where parameterDescription is parameterName (parameterType)[=defaultValue]: parameterDescriptionString
                           "wIn(int)=16: A first parameter - the input size; \
                            modulo(int): modulo; \
                            method(string)=modred: The method used for the montgomery algrithm."
